...
int main (int argc, char *argv[]) {
  ...
  char** gpu_luid = NULL;
  char** gpu_name = NULL;
  int ngpus = find_GPUs(myid, &gpu_luid, &gpu_name);
  ...
  ngpus_total = collect_GPUs(myid, nranks, ngpus, gpu_luid, gpu_name, &ngpus_all, &gpu_luid_all, &gpu_name_all);
  if (myid == 0) {
    if (ngpus_total > 0) {
	...
	idx += ngpus_all[r];
      ...
      parabort(0, ERR_TYPE_CUSTOM, 0, "No GPUs detected. Exiting...");
    } // ngpus_total
