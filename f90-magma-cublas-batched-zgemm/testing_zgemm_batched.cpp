...
// includes, custom
#include "wigner_d_matrix_acc.hpp"
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
	    ...
	    // Do NOT round up to multiple of 32
	    ldda = lda;
	    lddb = ldb;
	    lddc = ldc;
            ...
            /* Initialize Wigner matrix A on GPU using OpenACC and send it back to CPU */
	    // Find a suitable jmax
	    int jmax = floor(sqrt( (double)N )) - 1;
	    ...
	    /* Initialize random matrix B on CPU */
            lapackf77_zlarnv( &ione, ISEED, &sizeB, h_B );

            // lapackf77_zlarnv( &ione, ISEED, &sizeC, h_C );

	    /* Initialize empty matrix C on CPU */
#pragma omp parallel for collapse(3)
	    for( int s = 0; s < batchCount; s++ ) {
	      for( int i = 0; i < N; i++ ) {
		for( int j = 0; j < ldc; j++ ) {
		  h_C[s*ldc*N+i*ldc+j] = MAGMA_Z_MAKE( 0., 0. );
		}
	      }
	    }
